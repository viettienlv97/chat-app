...
export const updateUserSelfChat = async (req, res) => {
  try {
    const { userId } = req
    const {isOpenSelfChat} = req.body

    const user = await updateSelfChat(userId, isOpenSelfChat)
    if (!user) return invalidResponse(res, 400, 'Cannot find or update user')

    return dataResponse(res, 200, user)
  } catch (error) {
    console.log('Error in updateUserSelfChat', error)
    return serverResponse(res, 500, error)
  }
}
